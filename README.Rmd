...
`plot()` gives the network view, and `plot(x, "series")` the source series
the network was built from:

```{r hvg-network, fig.height = 4, fig.alt = "Horizontal visibility graph of a nine-point series, drawn as a spring-layout network with nodes sized by degree."}
network <- tsn(c(3, 1, 4, 2, 5, 3, 6, 2, 7), "hvg")

plot(network)                        # cograph network view
```

```{r hvg-series, fig.height = 3, fig.alt = "The nine-point source series the visibility graph was built from, plotted against time."}
...
```{r state-overlay, fig.height = 3.2, fig.alt = c("Daily step counts for one participant with horizontal shading marking the three quantile state bands across the y-axis.", "The same step series with vertical shading marking contiguous runs of each state along time.")}
data(steps)
state_network <- tsn(
  data = steps,
  value = "steps",
...
`"vertical"` shades contiguous state runs along time. Both use base graphics.
...
`series_networks()` returns a `tsn_series_networks` collection rather than a
bare list, so `print()`, `summary()`, `as.data.frame()`, and `plot()` all work
on it directly; `series` selects one model by name when the collection holds
more than one.
...
- `vignette("plotting-time-series-networks")` covers the plotting surface: the
  network and source-series views, state overlays, and transition-model plots.
