...
measurements: each series is discretized, a single state alphabet is
shared across all series, and the estimation is delegated to `Nestimate`
(a Suggests dependency). The four functions differ only in the quantity
placed on the edges. `ts_tna()` estimates transition probabilities,
`ts_ftna()` raw transition frequencies, `ts_cna()` co-occurrences, and
`ts_atna()` attention-weighted transitions.
...
the two students are placed on a common scale. `series_networks()` then
splits the pooled model into one network per series:
...
The resulting collection supports `print()`, `summary()`,
...
<img src="man/figures/README-transition-plot-1.png" alt="Two rows, one per student: each shows the daily effort series with a state ribbon strip underneath and that student's three-state transition network beside it, with edge weights printed on the arrows." width="100%" style="display: block; margin: auto;" />
...
`discretize()` is the bridge from a numeric series to a state sequence.
Using the same `srl` data, the student recorded as Erik contributes 156
daily reports:
...
`trend()` is a companion discretizer for *direction* rather than
*level*: it classifies each observation by the slope of a rolling
regression.
...
<img src="man/figures/README-visibility-network-1.png" alt="Natural visibility graph of sixty anxiety ratings, drawn with a Fruchterman-Reingold layout and nodes sized by degree; the graph traces a near-linear backbone with side branches, and a few high-degree hubs correspond to peak observations that see far along the series." width="100%" style="display: block; margin: auto;" />

``` r
plot(network, "series")
```

<img src="man/figures/README-visibility-series-1.png" alt="The sixty anxiety ratings the visibility graph was built from, plotted as a line over observation order." width="100%" style="display: block; margin: auto;" />

`vg(x)` builds a natural visibility graph and `vg(x, "horizontal")` a
horizontal one (Luque et al., 2009); the `tsn()` shortcuts `"nvg"` and
`"hvg"` are equivalent. Visibility options include `directed`,
`penetrable` (sightlines may cross a bounded number of points), `limit`
(a maximum elapsed time), and `decay`. When a `time` column is supplied,
both the sightlines and the elapsed-time rules respect the observed
spacing between points, which may be irregular, rather than mere row
positions.
...
<img src="man/figures/README-state-overlay-1.png" alt="Daily effort reports for the student Erik with horizontal shading marking the three quantile state bands across the y-axis." width="100%" style="display: block; margin: auto;" />
...
<img src="man/figures/README-distance-network-1.png" alt="Correlation-distance network of five momentary self-regulation indicators, drawn with labeled nodes; all ten pairs are connected and planning and effort form the strongest tie." width="100%" style="display: block; margin: auto;" />
...
- `vignette("constructing-time-series-networks")` applies every exported
  `tsn` function to one packaged momentary anxiety series, building each
  network representation in turn.
- `vignette("group-models")` builds one transition network per condition
  on a shared state alphabet and compares them.
- `vignette("nestimate-compatibility")` maps, verb by verb, which
  Nestimate analyses accept a `tsn` model directly.
...
- **`Nestimate`** (Saqr, López-Pernas, & Misiejuk, 2026) estimates and
  ...
  `ts_tna()` family discretizes the raw series and then calls its
  builders, so a `tsn` transition network is itself a `Nestimate` model
  that every `Nestimate` method applies to.
